...
import { Title, Text, Chip, Divider, Card, TextInput, Button } from "react-native-paper";
...
const AssignmentAudioScreen: React.FC<Props> = ({ route }) => {
    ...
    const renderColoredFeedback = (text: string, numberCorrectCode: string) => {
        const textList = text.split("").map((char, index) => index === 0 ? char.toUpperCase() : char);
        ...
        return (
            ...
            {textList.map((char, index) => (
                <Text
                key={index}
                style={correctCodeList[index] === "1" ? styles.coloredFeedbackGreen : styles.coloredFeedbackRed}
                >
                {char}
                </Text>
            ))}
    ...
    return (
        ...
        <View style={styles.card}>
            ...
            <>
              ...
              {renderColoredFeedback(currentQuestion?.answer || "", coloredCodeFeedback || "")}
