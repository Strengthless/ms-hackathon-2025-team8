...
const ForumScreen = () => {
  ...
  const [posts, setPosts] = useState<Post[]>(() =>
    forumPosts.map(
      (p: any, idx: number): Post => ({
        ...
        content: p.content ?? "",
        author: p.author ?? t("forum.anonymousParent", "Anonymous Parent"),
  ...
  const renderPost = ({ item }: ForumPostProps) => {
    ...
    const canComment = item.isPublic || isOwner || currentUser.role === "org";

    return (
      <Card style={styles.postCard}>
        <Card.Content>
          <View style={styles.postHeader}>
            <Avatar.Text size={40} label="AP" style={styles.avatar} />
          ...
          {!item.isPublic && isOwner && (
            <Text style={styles.privateHint}>
              {t("forum.privateHint", "Only you and the REACH can view this.")}
            </Text>
          )}
          ...
          <View style={styles.postFooter}>
            <View style={styles.reactionContainer}>
              <Button
                ...
                onPress={() => {
                  const updatedPosts = posts.map((p: Post) =>
                    p.id === item.id ? { ...p, likes: p.likes + 1 } : p
                  );
                  setPosts(updatedPosts);
                }}
              >
          ...
          {showReplyInput[item.id] && canComment && (
            <>
              <TextInput
                placeholder={t("forum.writeReply", "Write a reply...")}
          ...
          showNewPostForm ? (
            <Card style={styles.newPostCard}>
              <Card.Content>
                <Title>
                  {activeTab === "public"
                    ? t("forum.createPublic", "Create Public Discussion")
                    : t("forum.createPrivate", "Create Private Discussion")}
                </Title>
                <TextInput
                ...
                />
                <TextInput
                  label={t("forum.whatsOnMind", "What's on your mind?")}
...
const styles = StyleSheet.create({
  ...
  tabButton: { marginHorizontal: 8, flex: 1 },
  ...
  avatar: { backgroundColor: "#6200EE", marginRight: 12 },
  postInfo: { flex: 1 },
  ...
  replyAvatar: { backgroundColor: "#757575", marginRight: 8 },
  replyInfo: { flex: 1 },
  ...
  replyButton: { alignSelf: "flex-end", marginBottom: 8 },
});
